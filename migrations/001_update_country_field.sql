...
-- Step 2: Update existing records with full country names
-- Map existing country codes to full names
...
-- Step 5: Add NOT NULL constraint and update constraint
ALTER TABLE public.waitlist_registrations 
...
-- Step 7: Add check constraint for valid country names (optional - can be removed if too restrictive)
ALTER TABLE public.waitlist_registrations 
ADD CONSTRAINT check_country_length CHECK (char_length(country) >= 2 AND char_length(country) <= 100);

-- Note: The foreign key constraint on industry_id and other constraints remain unchanged